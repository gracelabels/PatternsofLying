...
v8_pf %>%
  ...
  arrange(desc(n)) %>%
  head(20) %>%
  kable(format = "pipe", caption = "Textual Ratings")
```
...
v9_pf %>%
  group_by(textualRating) %>%
  count() %>%
  kable(format = "pipe", caption = "Truth-O-Meter Rating Distribution")
```
...
v9_pf %>%
  ...
  arrange(desc(n)) %>%
  head(20) %>%
  kable(format = "pipe", caption = "Claimant Names by Number of Occurences")
```
...
v10_pf %>%
  ...
  arrange(desc(n)) %>%
  head(20) %>%
  kable(format = "pipe", caption = "Claimant Names by Number of Occurences, no duplicate names")
```
...
v11_pf %>%
  ...
  kable(format = "pipe", caption = "Claim Rating by Party for Final Dataset")
```
...
subjecttags <- read_csv("SubjectTagDef.csv")
