...
At this stage we noticed that some claimants were listed under several separate names referring to the same person. Like Speaker Nancy Pelosi, Nancy Pelosi, Speaker Pelosi. We wanted to eliminate this repetition so that we could see the true \# of claims made by each claimant. To do so, we moved our data into a program called OpenRefine. Here, we clustered claimant names by similar terms to identify where repetition/multiple names for the same person occured. We used this to identify all duplicate forms of name and then recoded the data accordingly.

This processed combined several names making the list 27 names shorter. 697 unique claimants remain.
...
Tucker Carlson, Laura Ingraham, Jacob Wohl, State representatives, Reagan was Right, and Marco Rubio's heckler. 21 claims by these claimants were removed as a result.
...
\
We decided to only tag False(ish) claims - Mostly False, False, and Pants on Fire. Our study is about what politicians are lying about\-\-- not what they are telling the truth about. We acknowledge this choice could limit our inquiry, but since claims were going to be tagged manually we decide to limit to only what we needed tagged. At this point, there were was 1,085 False(ish) claims to be tagged.
...
During tagging, 31 more claims were identified as duplicates, shown below. These were claims with differing urls but the same claim. These were not captured earlier because we searched for duplicates with identical urls. Most often these duplicates were fact-checks that had already been published and were included again in a round-up of fact-checks.
