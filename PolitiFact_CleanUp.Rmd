...
At this stage in the process, we were working with four publisher datasets from which we had sorted out all claims that did not fit our criteria of claims by U.S politician. We had also eliminated Independent and Libertarian claims. Now within each publisher we pivoted to focus on textualRating, which denotes the rating that the claim was given by the fact-checker. Some publishers have standardized scales with a ranking system, others do not. Here we defined the existing rating scale, identified how many claims fit into that scale, and how many anomaly claims remained.
...
There are: 2,527 normal claims (those rated with one of the six standardized ratings) and 354 anomalous claims (those with a non-standardized text based rating).
...
During this process we also noticed that some claimants who did not fit our defintion of politician, yet had not been identified as such by our code. To ensure that we had only the data we desired in our dataset, we ran the list of claimant names through a stricter version of the politician filter. 58 names were marked as potentially not being politicial figures. Each name was manually reviewed and we identified 6 names that did not belong in the dataset: Tucker Carlson, Laura Ingraham, Jacob Wohl, State representatives, Reagan was Right, and Marco Rubio's heckler.
...
Micro tags were traditional subject tags that describe the substance of the claim and the political issues or policy issues it is discussing.\
...
Through tagging, we wanted to be able to describe as many attributes of a claim as possible. First, we allowed a claim to be assigned multiple tags, and we had different types of tags-- Macro and Micro.
...
During tagging, 31 more claims were identified as duplicates, shown below. These were claims with differing urls but identical text. These were not captured earlier because we searched for duplicates with identical urls. Most often these duplicates were fact-checks that had already been published and were included again in an article round-up of fact-checks.
...
\
In addition to the Micro and Macro tags, we also tagged by the location of the lie, meaning the medium and format of the statement containing the lie. This tracked attributes such as whether it was said on TV, in an interview, on social media, etc. We had several overarching categories called `location` tags, and then more specific subcategories tagged `location.extra`. When loading this data into R, we made a select number of changes to make it compatible with the existing data.
