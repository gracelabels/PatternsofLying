...
```{r}
canonical <- read_csv("canonical_names.csv")
canonical$claimant <- canonical$claimant_canonical_name
originalnamespf <- read_csv("original name.csv")
```

```{r}
differences <- anti_join(originalnamespf, canonical, by = "claimant")

differences2 <- anti_join(canonical, originalnamespf, by = "claimant")
```
...
v11_mini <- v11_pf %>%
  filter(textualRating == "False"| textualRating == "Mostly False"| textualRating == "Pants on Fire")
          
         
anti_join(v11_mini, taggedclaims, by = c("...1", "url", "title", "languageCode", "publisher.name",
